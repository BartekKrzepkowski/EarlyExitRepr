...
def objective(exp, epochs, lr, wd, lr_lambda, init):
    ...
    N = 8
    model_params = {'layers_dim': [32*32*3] + [1024] * N + [NUM_CLASSES], 'activation_name': 'relu'}
    ...
    dataset_params = {'dataset_path': None, 'whether_aug': False, 'proper_normalization': True}
    ...
    GROUP_NAME = f'{exp}, {type_names["optim"]}, {type_names["dataset"]}, {type_names["model"]}_lr_{LR}_momentum_{MOMENTUM}_wd_{WD}_lr_lambda_{LR_LAMBDA}_hiddenN_{N}_init_{init}'
...
if __name__ == "__main__":
    ...
    lr_lambda = 1.0
    EPOCHS = 500
    objective('just_run', EPOCHS, lr, wd, lr_lambda, init)
