...
def objective(exp, epochs, lr, wd, lr_lambda):
    ...
    model = prepare_model(type_names['model'], model_params=model_params, init='kaiming_in').to(device)
    ...
    GROUP_NAME = f'{exp}, {type_names["optim"]}, {type_names["dataset"]}, {type_names["model"]}_lr_{LR}_momentum_{MOMENTUM}_wd_{WD}_lr_lambda_{LR_LAMBDA}_hiddenN_{N}'
